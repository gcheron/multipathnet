...
cmd:option('-checkexisting',false,'do not compute if an existing box result is found')
...
for im_i = config.start_id,#imagelist do
   ...
   if config.checkexisting then
      local f=io.open(boxreslist[im_i],"r")
      if f~=nil then io.close(f) ; compute = false ; end
   end
   if compute then
   ------------------- Run DeepMask --------------------
